      ...
      {editingTaskId && (() => { const et = projectTasks.find(t => t.id === editingTaskId); if (!et) return null; return (
        <Dialog open={!!editingTaskId} onOpenChange={(v) => { if (!v) setEditingTaskId(null); }}>
          <DialogContent className="sm:max-w-[420px] max-h-[80vh] overflow-y-auto space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div><label className="text-xs text-muted-foreground block mb-1">负责人</label><SimpleSelect value={et.leaderId || '__EMPTY__'} onValueChange={v => handleUpdateTask(et.id, { leaderId: v === '__EMPTY__' ? '' : v })} options={[{ value: '__EMPTY__', label: '未指定' }, ...activeMembers.map(m => ({ value: m.id, label: m.name }))]} className="w-full text-sm border border-input rounded-lg px-3 py-2 bg-card" /></div>
