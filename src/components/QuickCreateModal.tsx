      ...
      <div className="bg-card rounded-2xl shadow-2xl w-[480px] overflow-hidden" onClick={e => e.stopPropagation()} onKeyDown={handleKeyDown}>
        ...
        <div className="flex items-center gap-1 px-4 pt-4 pb-2">
          {(['task', 'goal', 'project'] as ItemType[]).map(t => (
            <button key={t} onClick={() => setType(t)} className={cn('flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors', type === t ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-muted')}>
        ...
        <div className="px-4 pb-3 flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-1.5">
            <label className="text-[10px] text-muted-foreground">{type === 'task' ? '截止日期' : '结束日期'}</label>
        ...
        <div className="px-4 py-2.5 border-t border-border bg-muted/30 flex items-center justify-between">
          <span className="text-[10px] text-muted-foreground">Ctrl+N 任务 | Ctrl+Shift+N 目标 | Ctrl+Shift+P 项目 | Enter 提交 | Esc 关闭</span>
          <button onClick={handleSubmit} disabled={!title.trim()} className="flex items-center gap-1 px-4 py-1.5 bg-primary text-primary-foreground rounded-lg text-xs font-medium hover:bg-primary/90 disabled:opacity-50">
