...
export default function Tasks() {
  ...
  const batchDelete = useCallback(() => { if (!can('tasks_delete')) return; if (!confirm(`确认删除选中的 ${selectedIds.size} 个任务？`)) return; const c = selectedIds.size; selectedIds.forEach(id => dispatch({ type: 'DELETE_TASK', payload: id })); setSelectedIds(new Set()); setBatchMode(false); try { window.dispatchEvent(new CustomEvent('tbh-toast', { detail: { message: `已删除 ${c} 个任务`, type: 'success' } })); } catch {} }, [selectedIds, dispatch]);
              ...
              {batchMode && selectedIds.size > 0 && (
                <div className="flex items-center gap-2 bg-primary/5 border border-primary/20 rounded-lg px-3 py-1.5 mr-1">
                  <span className="text-xs font-medium">已选 {selectedIds.size} 项</span>
                    <button className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={() => { if (!can('tasks_delete')) return; batchDelete(); }}><Trash2 size={12} /> 删除</button>
