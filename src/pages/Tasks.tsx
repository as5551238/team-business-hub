...
export default function Tasks() {
  ...
  const [newCategory, setNewCategory] = useState('');  useEffect(() => { try { localStorage.setItem(KANBAN_LS_KEY, JSON.stringify({ customMode: kanbanCustomMode, columns: customColumns, groupBy: kanbanGroupBy })); } catch (e) { handleError(e, { module: 'Tasks', operation: 'SAVE_KANBAN', severity: 'debug' }); } }, [kanbanCustomMode, customColumns, kanbanGroupBy]);
  ...
  function doCreateTask() {
    ...
    if (!title) return;
    dispatch({ type: 'ADD_TASK', payload: { title, description: (document.getElementById('new-task-desc') as HTMLTextAreaElement)?.value || '', projectId: newProjectId || null, goalId: newGoalId || null, parentId: newParentId || null, status: 'todo' as TaskStatus, priority: (newPriority || 'medium') as TaskPriority, leaderId: newLeaderId || state.currentUser?.id || '', supporterIds: Array.from(newSupporters), tags: Array.from(newTags), category: newCategory || '', startDate: (document.getElementById('new-task-start') as HTMLInputElement)?.value || null, dueDate: (document.getElementById('new-task-due') as HTMLInputElement)?.value || null, reminderDate: null, completedAt: null, subtasks: [], attachments: [], trackingRecords: [], repeatCycle: 'none', summary: '' } });
    closeCreateDialog();
  }
              ...
              {!fromTemplate ? (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div><label className="text-sm font-medium mb-1.5 block">紧急程度</label><SimpleSelect value={newPriority} onValueChange={setNewPriority} options={[{ value: 'urgent', label: '紧急 (S)' }, { value: 'high', label: '高 (A)' }, { value: 'medium', label: '中 (B)' }, { value: 'low', label: '低 (C)' }]} className="w-full h-9 text-sm" /></div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-sm font-medium mb-1.5 block">关联目标</label><SimpleSelect value={newGoalId || '__EMPTY__'} onValueChange={v => setNewGoalId(v === '__EMPTY__' ? '' : v)} options={[{ value: '__EMPTY__', label: '无' }, ...state.goals.filter(g => g.status === 'in_progress').map(g => ({ value: g.id, label: g.title }))]} className="w-full h-9 text-sm" /></div>
                    <div><label className="text-sm font-medium mb-1.5 block">所属项目</label><SimpleSelect value={newProjectId || '__EMPTY__'} onValueChange={v => setNewProjectId(v === '__EMPTY__' ? '' : v)} options={[{ value: '__EMPTY__', label: '无' }, ...state.projects.filter(p => p.status === 'in_progress').map(p => ({ value: p.id, label: p.title }))]} className="w-full h-9 text-sm" /></div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-sm font-medium mb-1.5 block">主导人</label><SimpleSelect value={newLeaderId || '__EMPTY__'} onValueChange={v => setNewLeaderId(v === '__EMPTY__' ? '' : v)} options={[{ value: '__EMPTY__', label: '未指定' }, ...activeMembers.map(m => ({ value: m.id, label: m.name }))]} className="w-full h-9 text-sm" /></div>
