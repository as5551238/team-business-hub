...
import { PROVIDER_PRESETS, loadAIConfig, COST_ROUTING_MAP, detectTaskComplexity, BUILTIN_API_KEY, BUILTIN_BASE_URL, BUILTIN_MODEL } from './types';
import { getSupabaseClient } from '@/supabase/client'; // TDZ fix: static import instead of dynamic import

/** 内置 Key 常量（llmService 内部使用） */
const _BUILTIN_KEY = BUILTIN_API_KEY;
const _BUILTIN_URL = BUILTIN_BASE_URL;
const _BUILTIN_MODEL = BUILTIN_MODEL;

/** 确保 config 有可用凭证：用户没填时使用内置 Key */
function ensureCredentials(config: AIConfig): AIConfig {
  if (!config.apiKey) config = { ...config, apiKey: _BUILTIN_KEY };
  if (!config.baseUrl) config = { ...config, baseUrl: _BUILTIN_URL };
  if (!config.model) config = { ...config, model: _BUILTIN_MODEL };
  return config;
}

/** 两步代理调用：submit → 轮询 poll（pg_net 异步模式） */
async function callLLMViaProxy(url: string, body: string, apiKey: string): Promise<string | null> {
  const sb = getSupabaseClient();
  if (!sb) throw new Error('Supabase 客户端未初始化');

  // Step 1: 提交请求
  const { data: requestId, error: submitErr } = await sb.rpc('call_llm_proxy_submit', {
    p_url: url,
    p_body: body,
    p_api_key: apiKey,
  });
  if (submitErr || !requestId) {
    throw new Error('代理提交失败: ' + (submitErr?.message || '无requestId'));
  }

  // Step 2: 轮询结果（最多15次，每次等1s）
  for (let i = 0; i < 15; i++) {
    await new Promise(r => setTimeout(r, 1000));
    const { data, error: pollErr } = await sb.rpc('call_llm_proxy_poll', {
      p_request_id: requestId,
    });
    if (pollErr) throw new Error('代理轮询失败: ' + pollErr.message);
    if (data === null) continue; // 还在等
    if (data.error) throw new Error('代理返回错误: ' + data.error);
    if (data.status_code === 200 && data.content) {
      // content 可能是 JSON string
      try {
        const parsed = JSON.parse(data.content);
        return parsed.choices?.[0]?.message?.content || data.content;
      } catch {
        return data.content;
      }
    }
    if (data.status_code) {
      throw new Error(`代理返回 HTTP ${data.status_code}`);
    }
  }
  throw new Error('代理轮询超时(15s)');
}
...
export async function callLLM(prompt: string, config: AIConfig, complexity?: TaskComplexity): Promise<string | null> {
  // 确保有凭证（内置Key兜底）
  config = ensureCredentials(config);
  ...
  try {
    ...
    // Strategy 2: Supabase RPC 两步代理（绕过 CORS/CSP）
    ...
    if (isNetworkError || isTimeout) {
      ...
      try {
        const result = await callLLMViaProxy(url, JSON.stringify(requestBody), config.apiKey);
        if (result) {
