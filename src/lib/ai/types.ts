...
export function loadAIConfig(): AIConfig {
  try {
    ...
    if (s) {
      ...
      // 迁移空配置：用内置值填充
      if (!saved.apiKey) { saved.apiKey = BUILTIN_API_KEY; migrated = true; }
      if (!saved.baseUrl) { saved.baseUrl = BUILTIN_BASE_URL; migrated = true; }
      if (!saved.model) { saved.model = BUILTIN_MODEL; migrated = true; }
...
export function loadPlanTier(): string {
  try { return localStorage.getItem('tbh-plan-tier') || 'free'; } catch { return 'free'; }
}

export function loadTeamId(): string {
  try { return localStorage.getItem('tbh-team-id') || ''; } catch { return ''; }
}
