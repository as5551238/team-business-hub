...
describe('calcGoalProgress', () => {
  ...
  it('简单平均：2个KR各50%完成 = 50', () => {
    const goals = [makeGoal({
      keyResults: [
        { id: 'kr1', title: 'KR1', currentValue: 5, targetValue: 10, unit: '%', weight: 1, selected: true },
        { id: 'kr2', title: 'KR2', currentValue: 50, targetValue: 100, unit: '%', weight: 1, selected: true },
      ],
    })];
  ...
  it('加权平均：KR1(weight3,50%) + KR2(weight1,100%) = 62.5%→63', () => {
    const goals = [makeGoal({
      keyResults: [
        { id: 'kr1', title: 'KR1', currentValue: 5, targetValue: 10, unit: '%', weight: 3, selected: true },
        { id: 'kr2', title: 'KR2', currentValue: 100, targetValue: 100, unit: '%', weight: 1, selected: true },
      ],
    })];
  ...
  it('selected过滤：仅selected的KR参与计算', () => {
    const goals = [makeGoal({
      keyResults: [
        { id: 'kr1', title: 'KR1', currentValue: 8, targetValue: 10, unit: '%', weight: 1, selected: true },
        { id: 'kr2', title: 'KR2', currentValue: 0, targetValue: 10, unit: '%', weight: 1, selected: false },
      ],
    })];
    // 只有 kr1 参与: 8/10 = 80%
  ...
  it('全部KR未selected时返回0', () => {
    const goals = [makeGoal({
      keyResults: [
        { id: 'kr1', title: 'KR1', currentValue: 8, targetValue: 10, unit: '%', weight: 1, selected: false },
      ],
    })];
    expect(calcGoalProgress(goals, 'g1')).toBe(0);
  });
  ...
  it('子目标平均：2个子目标各50%/100% = 75', () => {
    const goals = [
      ...
      makeGoal({
        id: 'g2', parentId: 'g1',
        keyResults: [{ id: 'kr1', title: 'KR', currentValue: 5, targetValue: 10, unit: '%', weight: 1, selected: true }],
      }),
      makeGoal({
        id: 'g3', parentId: 'g1',
        keyResults: [{ id: 'kr2', title: 'KR', currentValue: 10, targetValue: 10, unit: '%', weight: 1, selected: true }],
      }),
    ];
    // g2=50%, g3=100%, avg=75%
...
describe('calcProjectProgress', () => {
  ...
  it('done=100%, todo=0%, 平均=50%', () => {
    const tasks = [
      { id: 't1', projectId: 'p1', status: 'done', subtasks: [] } as unknown as Task,
      { id: 't2', projectId: 'p1', status: 'todo', subtasks: [] } as unknown as Task,
    ];
  ...
  it('in_progress无子任务时算50%', () => {
    const tasks = [
      { id: 't1', projectId: 'p1', status: 'in_progress', subtasks: [] } as unknown as Task,
    ];
    expect(calcProjectProgress(tasks, 'p1')).toBe(50);
  });
  ...
  it('混合状态：done(100) + in_progress+75%子任务(75) + todo(0) = 58.3→58', () => {
    const tasks = [
      ...
      { id: 't3', projectId: 'p1', status: 'todo', subtasks: [] } as unknown as Task,
    ];
    // (100 + 75 + 0) / 3 = 58.33 → 58
  ...
  it('只计算指定项目的任务', () => {
    const tasks = [
      { id: 't1', projectId: 'p1', status: 'done', subtasks: [] } as unknown as Task,
      { id: 't2', projectId: 'p2', status: 'todo', subtasks: [] } as unknown as Task,
    ];
