...
export function commentReducer(state: AppState, action: Action): AppState | null {
  switch (action.type) {
    case 'ADD_COMMENT': {
      ...
      const comment = { ...action.payload, teamId: s.currentTeamId || '__default__', id: genId('c'), createdAt: new Date().toISOString(), content: clampComment(action.payload.content) || '' };
    ...
    case 'UPDATE_COMMENT': {
      ...
      const s = needMutate(state, ['comments']);
      const now = tsNow();
      s.comments = s.comments.map(c => c.id === action.payload.id ? { ...c, ...action.payload.updates, updatedAt: now } : c);
      supabaseUpdate('comments', action.payload.id, { ...action.payload.updates, updated_at: now });
      return s;
    }
  }
  return null;
