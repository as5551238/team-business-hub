...
export type Action =
  ...
  | { type: 'ADD_ITEM_LINK'; payload: Omit<ItemLink, 'id' | 'teamId' | 'createdAt'> }
  ...
  | { type: 'ADD_SPRINT'; payload: Omit<Sprint, 'id' | 'teamId' | 'createdAt' | 'updatedAt'> }
  | { type: 'UPDATE_SPRINT'; payload: { id: string; updates: Partial<Sprint> } }
  | { type: 'DELETE_SPRINT'; payload: string }
  | { type: 'ADD_KNOWLEDGE'; payload: Omit<Knowledge, 'id' | 'teamId' | 'createdAt' | 'updatedAt'> }
...
export function ensureAppStateDefaults(data: Partial<AppState> & { members: Member[] }): AppState {
  ...
  result.categories = result.categories.map((c: Category) => ({
    ...c, teamId: c.teamId ?? '__default__', appliesTo: c.appliesTo ?? [], color: c.color ?? '#6366f1',
    icon: c.icon ?? 'tag',
  }));
  result.tags = result.tags.map((t: Tag) => ({
    ...t, teamId: t.teamId ?? '__default__', color: t.color || '#6366f1',
  }));
  result.notes = result.notes.map((n: Note) => ({
    ...n, teamId: n.teamId ?? '__default__', tags: n.tags ?? [], category: n.category ?? '',
  }));
  result.comments = result.comments.map((c: Comment) => ({
    ...c, teamId: c.teamId ?? '__default__', mentionedMemberIds: c.mentionedMemberIds ?? [],
  ...
  result.scheduleEvents = result.scheduleEvents.map((e: ScheduleEvent) => ({
    ...e, teamId: e.teamId ?? '__default__', repeatCycle: e.repeatCycle ?? 'none', allDay: e.allDay ?? true, memberId: e.memberId ?? '',
  }));
  result.bookmarks = result.bookmarks.map((b: Bookmark) => ({
    ...b, teamId: b.teamId ?? '__default__', icon: b.icon ?? 'file', category: b.category ?? '默认', order: b.order ?? 0, memberId: b.memberId ?? '',
  }));
  result.savedViews = result.savedViews.map((v: SavedView) => ({
    ...v, teamId: v.teamId ?? '__default__', filters: v.filters ?? [], filterLogic: v.filterLogic ?? 'and', memberId: v.memberId ?? '', updatedAt: v.updatedAt || v.createdAt || '',
  }));
  result.templates = result.templates.map((t: Template) => ({
    ...t, teamId: t.teamId ?? '__default__', isPublic: t.isPublic ?? true, category: t.category ?? '',
  }));
  result.activities = result.activities.map((a: Activity) => ({
    ...a, details: a.details ?? '',
  }));
  result.reviews = result.reviews.map((r: ReviewEntry) => ({
    ...r, teamId: r.teamId ?? '__default__', content: r.content ?? '', improvements: Array.isArray(r.improvements) ? r.improvements : [], metrics: r.metrics && typeof r.metrics === 'object' ? r.metrics : { goalsCompleted: 0, goalsInProgress: 0, projectsCompleted: 0, projectsInProgress: 0, tasksCompleted: 0, tasksOverdue: 0, tasksTotal: 0, completionRate: 0 },
  }));
  ...
  result.sprints = result.sprints.map((sp: any) => ({
    ...sp, teamId: sp.teamId ?? '__default__', goalIds: sp.goalIds ?? (sp.goal_ids as string[]) ?? [], status: sp.status ?? 'planning',
  }));
  result.knowledge = result.knowledge.map((k: any) => ({
    ...k, teamId: k.teamId ?? '__default__', tags: k.tags ?? [], relatedItems: k.relatedItems ?? (k.related_items as Knowledge['relatedItems']) ?? [], content: k.content ?? '',
  }));
  result.itemLinks = result.itemLinks.map((l: ItemLink) => ({
    ...l, teamId: l.teamId ?? '__default__',
  }));
  return result;
}
